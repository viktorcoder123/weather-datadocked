-- Create ports table for CSV import (handles empty coordinates)
...
CREATE INDEX IF NOT EXISTS idx_ports_unlocode ON public.ports(un_locode);
CREATE INDEX IF NOT EXISTS idx_ports_country ON public.ports(country);
CREATE INDEX IF NOT EXISTS idx_ports_location ON public.ports(latitude, longitude) WHERE latitude IS NOT NULL AND longitude IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_ports_active ON public.ports(is_active);
CREATE INDEX IF NOT EXISTS idx_ports_with_coords ON public.ports(port_name) WHERE latitude IS NOT NULL AND longitude IS NOT NULL;
